...
<?php foreach ( $matches as $index => $record ) { ?>
		<li class="item-<?php echo intval( $index ) + 1 ?>">
			...
			<span class="actions">
				<a class="show" href="<?php echo $relation->unboundEnd()->getterUrl( $record ) ?>"><?php echo _L('Show') ?></a>
<?php if ( user::current()->isAuthenticated() && $delete  ) { ?>
				<a class="delete" href="<?php echo $relation->managing()->getterUrl( $record, array( 'action' => 'delete' ) ) ?>"><?php echo _1($delete !== true ? $delete : null,_L('Delete')) ?></a>
<?php } ?>
			</span>
		</li>
<?php } ?>
...
<?php if ( user::current()->isAuthenticated() && $add ) { ?>
	<span class="actions">
		<a class="add" href="<?php echo $relation->managing()->getterUrl( null, array( 'action' => 'add' ) ) ?>"><?php echo _1($add !== true ? $add : null,_L('Add')) ?></a>
	</span>
<?php } ?>
</div>
<?php }
